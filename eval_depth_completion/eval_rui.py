...
def main():
    ...
    for data in tqdm(dataset):
        ...
        # depth_pred = cv2.imread(data_folder / sc / "depthL.png", cv2.IMREAD_UNCHANGED)
        # depth_pred = (depth_pred.astype(float)) / 1000.0
        # depth_pred = cv2.resize(depth_pred, (960, 540), interpolation=cv2.INTER_NEAREST)
        ...
        if args.gt_normal:
            ...
            normal_gt = np.transpose(normal_gt, (2, 0, 1))
            normal_gt2 = np.stack([normal_gt[0], normal_gt[2], -normal_gt[1]])
            with h5py.File(view_folder / "gt_normal.h5","w") as f:
                f.create_dataset("result", data=normal_gt2)

        else:
            normal_pred = np.load(f"/media/DATA/LINUX_DATA/activezero2/realnormal/normal/{sc}.npy")
            normal_pred2 = np.stack([normal_pred[0], -normal_pred[1], -normal_pred[2]])
            with h5py.File(view_folder / "pred_normal.h5", "w") as f:
                f.create_dataset("result", data=normal_pred2)
            normal_pred3 = np.stack([normal_pred[0], normal_pred[2], -normal_pred[1]])
            normal_pred3 = np.transpose(normal_pred3, (1, 2, 0))
            normal_diff = np.arccos(np.clip(np.sum(normal_fitting * normal_pred3, axis=-1), -1, 1))
            normal_mask = normal_diff < args.normal_threshold / 180 * np.pi
            cv2.imwrite(view_folder / "normal_mask.png", (normal_mask.astype(np.uint8)) * 255)
        ...
        o = (
            ...
            f" -inertia_weight 100 -smoothness_weight 0.001 -tangent_weight 1"
        )
        if args.gt_normal:
